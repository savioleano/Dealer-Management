...
export default function EditManagerForm({ manager }: { manager: Manager }) {
  const router = useRouter()
  const [form, setForm] = useState({ name: manager.name, email: manager.email, phone: manager.phone })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) {
      ...
      setError(data.error ?? 'Failed to update manager.')
      setSaving(false)
      return
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
        <div className="text-xs font-medium text-gray-400 uppercase tracking-wide">User Level: Manager</div>
      ...
      <div className="flex justify-end gap-3">
        <button type="button" onClick={() => router.push('/admin/managers')}
